...
parser.add_argument("--filename", help="Output file", type=str, default="Experiments_part1.json")
...
def sample_hyperparameters():
    ...
    #Define architecture
    num_hidden_layers = np.random.randint(1, 5+1) #[1, 5]

    if num_hidden_layers == 1:
        scheme = 'same'
    elif num_hidden_layers == 2:
        scheme = np.random.choice(['same', 'rise']) 
    else:
        scheme = np.random.choice(['same', 'rise', 'rise_decrease'])
    ...
    if regularization_type in ['l1', 'l2']:
        regularization_strength = loguniform.rvs(1e-5, 1e-1)
    elif regularization_type == 'dropout':
        regularization_strength = loguniform.rvs(1e-4, 1e-1)
    ...
    hyperparams_net = {
        ...
        'activation_func' : np.random.choice(['ReLU', 'Tanh', 'Sigmoid', 'LeakyReLU']),
    ...
    hyperparams_learn = {
        ...
        'optim' : np.random.choice(['Adam', 'RMSprop', 'SGD']),
        'loss': 'MSELoss',
        'folds': 4,
